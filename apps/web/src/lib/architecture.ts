...
export function buildStages({ config, health, llm }: LiveInputs): Stage[] {
  ...
  return [
    ...
    },
    {
      id: 'search',
      lane: 'resposta',
      title: 'Busca adicional',
      summary: 'Se os trechos não bastam, o modelo pede <buscar>termos</buscar> e a busca roda de novo — até 2 rodadas',
      details: [
        'Na configuração com busca adicional, o prompt autoriza o modelo a NÃO responder ainda: se faltar o item certo, um valor ou uma condição, ele devolve só linhas <buscar>termos</buscar> (até 3), com as palavras que procuraria no edital.',
        ...
        'Mira os erros de recuperação (a pergunta usa um vocabulário e o edital outro). Custa latência só quando é acionada: nas execuções finais, em cerca de uma a cada três perguntas com o Sonnet e uma a cada cinco com o Haiku.',
      ],
      params: [['ligado', c.generation.extraSearch ? 'sim' : 'só na configuração com busca adicional'], ['rodadas', '2'], ['buscas por rodada', 'até 3'], ['top-k da busca adicional', '6'], ['limite de contexto extra', '10.000 chars']],
...

    {
      ...
      summary: 'Escada: documento inteiro · vetorial · BM25 · híbrido · só folhas · + expansão · + gate (configuração híbrida) · + busca adicional',
      details: [
'Os braços formam uma escada lida por pares, e cada par difere em um componente só: documento inteiro vs RAG híbrido sem gate mede a recuperação (os dois com o gate apenas observando); RAG vetorial e RAG léxico BM25, cada um contra o híbrido, medem a fusão RRF; híbrido vs só folhas mede a expansão folha→pai; híbrido vs híbrido + expansão de consulta mede a reformulação da pergunta; expansão vs configuração híbrida mede o gate de fundamentação. Documento inteiro vs configuração híbrida muda três coisas ao mesmo tempo (recuperação, expansão e gate) e vale como comparação de ponta a ponta. Só o último braço usa o gate; nos demais ele apenas observa (política warn), então a resposta entregue é a que um RAG comum entregaria.',
        'Fora da escada ficam a configuração com busca adicional (buscas dirigidas pelo modelo), o contexto reduzido (top-4) e o controle sem documento, que não é configuração válida do produto — só mede o quanto o modelo inventa sem fonte. Todos passam pelo mesmo validador de citações e pelo mesmo pontuador, então os números são comparáveis; os resultados saem no geral e por documento do dataset.',
        'Na apresentação: documento inteiro (teto), RAG vetorial (o RAG clássico), configuração híbrida e configuração com busca adicional, todos com o mesmo modelo, o mesmo prompt e as mesmas variantes de expansão por pergunta.',
      ],
